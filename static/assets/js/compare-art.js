...
function renderArtsInCompareBoard() {
  // render arts in compare board
  var selectedArts = $(
    ".carousel-container .carousel-item.selected .art-img-item-wrapper"
  );
  $(".art-wrapper").empty();
  ...
  if (selectedArts.length > 0) {
    ...
    for (var i = 0; i < selectedArts.length; i++) {
      $(".magnifying-result-container .result-img-container").append(`
        <div class="result-img">
          <div class="art-ele" style="background-image: url(${
            $(selectedArts[i]).find("img")[0].src
          })"></div>
        </div>
      `);
    ...
    // magnifying result
    $(".magnifying-result-container .result-img-container").append(
      `<span>There is no selected art.</span>`
    );
  }
  //
  if (
...
$(document).on("change", ".view-option input[name=viewOption]", function () {
  ...
  $(".view-options")
    .find(
      "input[value=layered-toggle-view], input[value=layered-side-by-side], input[value=magnify]"
    )
    .prop("checked", false);
  // check selected view option
  $(".view-options")
    .find(`input[name=viewOption][value=${this.value}`)
    .prop("checked", true);
...
$(document).on(
  "click",
  ".btn-toggle, .layered-toggle-view .art-img",
  function () {
    $(".art-wrapper").toggleClass("toggled");
    $("#fullscreenContent.layered-toggle-view .result-img-names").toggleClass(
      "toggled"
    );
    $(
      "#fullscreenContent.layered-toggle-view .result-img-container"
    ).toggleClass("toggled");
  }
);
...
$(document).on(
  ...
  function (e) {
    ...
    //
    var zoomPosX = mouseX - wrapperX,
      zoomPosY = mouseY - wrapperY;
    // var mgX = mouseX - wrapperX - 36, mgY = mouseY - wrapperY - 36;
    var mgX = mouseX - wrapperX - 40 / 2,
      mgY = mouseY - wrapperY - 40 / 2;
    ...
    $(".magnifying-result .art-ele").css({
      "background-position": `${(100 * zoomPosX) / artW}% ${
        (100 * zoomPosY) / artH
      }%`,
    });
  }
...
function adjustFullscreenModalSize() {
  ...
  if (sWidth > sHeight) {
    var mmW = $("#fullScreenModal").hasClass("magnify")
      ? sHeight
      : sHeight - 168;
  ...
  if (selectedArts.length === 2) {
    ...
    for (var i = 0; i < selectedArts.length; i++) {
      $("#fullScreenModal .fullscreen-result .result-img-names").append(`
        <div class="art-name">${$(selectedArts[i])
          .find(".art-name")
          .text()}</div>
      `);
    }
...
function initMagnifyModal() {
  ...
  if (selectedArts.length > 0) {
    ...
    $("#fullScreenModal .fullscreen-result .result-img-container").append(
      `<span>There is no selected art.</span>`
    );
  }
  resizeZoomedImages();
...
$(document).on(
  ...
  function (e) {
    ...
    $("#fullScreenModal .fullscreen-result .art-ele").css({
      "background-position": `calc(${(100 * zoomPosX) / artW}% + ${
        8 * (realZoom - 1)
      }px) calc(${(100 * zoomPosY) / artH}% + ${8 * (realZoom - 1)}px)`,
    });
  }
);
...
function resizeZoomedImages() {
  ...
  sMZoomedImg.css({
    "background-size": `${realZoom * 100}% ${realZoom * 100}%`,
  });
  fMZoomedImg.css({
    "background-size": `${realZoom * 100}% ${realZoom * 100}%`,
  });
